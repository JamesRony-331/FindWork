...
test('filterRows 在指定字段中进行不区分大小写的包含匹配', () => {
  const rows = [{ job: 'Java 开发', city: '杭州' }, { job: '测试工程师', city: '上海' }]
...
export function paginateRows(rows, page, pageSize) {
  ...
  return { rows: rows.slice(start, start + pageSize), total: rows.length, page: safePage, pageCount }
}
...
test('薪资分析包含文档要求的五类可视化', () => {
  ...
  for (const title of ['岗位平均薪资', '薪资区间分布', '城市平均薪资排行', '不同学历平均薪资', '高薪岗位 Top10']) {
...
test('岗位查询按关键词过滤并重置到第一页', () => {
  ...
  state.search()
  assert.equal(state.visibleRows.every((row) => row.jobName.includes('Java')), true)
