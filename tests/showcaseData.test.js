...
test('主页提供进入系统展示页的按钮', () => {
  ...
  assert.deepEqual(featuredAction, { label: '查看系统展示', path: '/showcase', primary: true, featured: true })
})

test('系统展示页由结构化数据提供四个主题分区', () => {
  assert.deepEqual(showcaseSections.map((item) => item.key), ['demand', 'salary', 'city', 'ai'])
